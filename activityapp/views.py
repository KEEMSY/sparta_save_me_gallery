...
def home(request):
    if request.method == "GET":
        return render(request, "index.html", {"activities": get_activity_page_load(1)})
    # 삭제하는 경우
    else:
        # 삭제하는 함수
...
def choice_activity_page(request):
    ...
    return render(
        request, "activityapp/choice.html", {"all_activities": all_activities}
    )
...
def a_choice_activity_page(request):
    return render(
        request,
        "activityapp/model1.html",
        {"all_kind_of_activities": Info.objects.all()},
    )
